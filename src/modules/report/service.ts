...
const summaryForRange = async (userId: string, start: Date, end: Date) => {
  const [income, expense] = await Promise.all([
    ...
    prisma.transaction.aggregate({ _sum: { amount: true }, where: { userId, type: 'EXPENSE', date: { gte: start, lt: end } } }),
  ]);
...
export const yearly = async (userId: string, year: number) => {
  ...
  return cached(key, async () => summaryForRange(userId, new Date(year, 0, 1), new Date(year + 1, 0, 1)));
};

export const categoryBreakdown = async (userId: string, month: number, year: number) => {
  ...
  return cached(key, async () => {
    ...
    return groups.map((group) => ({
      ...
      categoryName: categories.find((category) => category.id === group.categoryId)?.name ?? 'Unknown',
...
export const exportReport = async (userId: string, type: 'pdf' | 'csv', month: number, year: number) => {
  const data = await monthly(userId, month, year);
  if (type === 'csv') {
    return { contentType: 'text/csv', filename: `report-${year}-${month}.csv`, body: generateCsv([data]) };
  }
  ...
  return { contentType: 'application/pdf', filename: `report-${year}-${month}.pdf`, body };
};
